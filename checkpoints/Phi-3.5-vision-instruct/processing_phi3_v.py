...
from ARegionCD.src.highlighter_modules.utils import txt_highlight_mask
...
class Phi3VImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
            self,
            images: ImageInput,
            images_mask: Union[Image.Image, np.ndarray, List[Image.Image], List[np.ndarray]] = None,
            bboxes: Optional[List[Tuple[float, float, float, float]]] = None,
            mask_values: Optional[Union[int, List[int]]] = None,
        ...
        image_sizes = shapes  # [torch.LongTensor(_shapes) for _shapes in shapes]
        padded_images = image_transformed

        bbox_masks = []
        if bboxes is not None:
            for image, hd_image, bbox, mask_img, mask_value, attr in zip(images, hd_images, bboxes, images_mask,
                                                                         mask_values, attribute):
                W, H = image.size
                _, nH, nW = hd_image.shape

                if attr in {"whole", "merge"}:
                    mask_value = 128
                    ...
                    image_mask[y_min:y_max, x_min:x_max] = mask_value
                    mask = np.where(image_mask == mask_value, 1, 0).astype(np.float32)

                else:
                    mask = np.where(mask_img == mask_value, 1, 0).astype(np.float32)
        ...
        data = {
            ...
            "bbox_attention_mask": bbox_masks,
            "shapes": shapes,
        }
...
class Phi3VProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        if images is not None:
            ...
            # 改
            # image_inputs = self.image_processor(images, bboxes=bboxes, return_tensors=return_tensors)
            image_inputs = self.image_processor(images, images_mask=images_mask, bboxes=bboxes, mask_values=mask_values,
                                                attribute=attribute, return_tensors=return_tensors)
        else:
            image_inputs = {}
    ...
    def _convert_images_texts_to_inputs(self, images, texts, padding=False, truncation=None, max_length=None,
        ...
        if 'bbox_attention_mask' in images:
            bbox_attention_mask = images['bbox_attention_mask']
        else:
            bbox_attention_mask = None
        ...
        data = {
            ...
            "image_ids_pad": image_ids_pad,
            "shapes": shapes
        }

        if bbox_attention_mask is not None:
            data["bbox_attention_mask"] = bbox_attention_mask
